...
class PayPalClient:
    ...
    async def get_access_token(self, client: httpx.AsyncClient) -> str:
        ...
        try:
            ...
            logger.error("Failed to Authenticate to PayPal")
            raise HTTPException(
            ...
            logger.error("An error occurred while connecting to PayPal")
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail=f"Unable to connect to PayPal",
            )

    async def create_order(self, payload:dict[str,str]):
    ...
    async def capture_order(self, order_id: str):
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            ...
            url = f"{self.base_url}" f"/v2/checkout/orders/{order_id}/capture"

            try:

                response = await client.post(url, headers=self._auth_headers(token))
