...
function autoBind(instance: Record<string, unknown>): void {
  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    ...
    if (typeof val === "function") {
      ...
      (instance as Record<string, AnyFunction>)[key] = (
        val as AnyFunction
      ).bind(instance);
    }
  });
  ...
  createShader() {
    ...
    if (this.priority) {
      loadImage(this.image)
        .then(applyImage)
        .catch((err) =>
          console.error(`Failed to load image at ${this.image}`, err),
        );
    } else {
      const idle = (
...
class App {
  ...
  createMedias(
    ...
    const defaultItems = [
      ...
      },
      {
        ...
        text: "Chasing birdles and dreams",
      },
      {
        image: "/peacefound.jpg",
        text: "Peace found on the fairway",
      },
      {
        image: `/puttingittowork.jpg`,
        text: "Putting it to work",
      },
      {
        image: "/galleryimage3.jpg",
        text: "Sun, swings and smiles",
      },
      {
        image: `/galleryimage4.jpg`,
        text: "Golf is my therapy",
      },
      {
        image: `/dressedtotee.jpg`,
        text: "Always dressed to tee",
      },
      {
        image: `/par-teetime.jpg`,
        text: "Par-tee time!",
      },
      {
        image: `/grassgreener.jpg`,
        text: "Where the grass is always greener",
      },
      {
        image: `/gripit.jpg`,
        text: "Grip it sip it",
  ...
  onWheel(e: Event) {
    ...
    const legacyWheel = wheelEvent as unknown as {
      wheelDelta?: number;
      detail?: number;
    };
    const delta: number = (wheelEvent.deltaY ??
      legacyWheel.wheelDelta ??
      legacyWheel.detail ??
      0) as number;
    this.scroll.target +=
...
interface CircularGalleryCoreProps {
  items?: { image: string; text: string }[];
  bend?: number;
  textColor?: string;
  borderRadius?: number;
